...
class UtilsTest(unittest.TestCase):
    ...
    def test_write_csebbs_inputs_and_apply_csebbs(self):
        predictions = {
            "b": pd.DataFrame(
                {
                    "onset": [0.2, 0.0, 0.1],
        ...
        ground_truth = {"b": [(0.0, 0.2, "bell")]}
        durations = {"b": 0.3}

        with tempfile.TemporaryDirectory() as temp_dir:
            scores_dir, ground_truth_path, durations_path = write_csebbs_inputs(
                predictions,
                ground_truth,
                durations,
                temp_dir,
            )
            self.assertTrue((Path(scores_dir) / "file_b.tsv").exists())

            with open(ground_truth_path, newline="") as handle:
                rows = list(csv.DictReader(handle, delimiter="\t"))
            self.assertEqual(rows, [{"filename": "file_b.wav", "onset": "0.0", "offset": "0.2", "event_label": "bell"}])

            with open(durations_path, newline="") as handle:
                rows = list(csv.DictReader(handle, delimiter="\t"))
            self.assertEqual(rows, [{"filename": "file_b.wav", "duration": "0.3"}])

        class FakeCsebbsPredictor:
            def predict(self, scores_dict):
                self.received_onsets = scores_dict["b"]["onset"].tolist()
                return {"b": [(0.0, 0.2, "bell"), (0.1, 0.3, "coffee", 0.6)]}

        predictor = FakeCsebbsPredictor()
        converted = apply_csebbs(predictions, predictor, CLASS_NAMES)

        self.assertEqual(predictor.received_onsets, [0.0, 0.1, 0.2])
        self.assertEqual(converted["b"]["onset"].tolist(), [0.0, 0.1, 0.2])
        self.assertEqual(converted["b"]["bell"].tolist(), [1.0, 1.0, 0.0])
        self.assertEqual(converted["b"]["coffee"].tolist(), [0.0, 1.0, 1.0])
